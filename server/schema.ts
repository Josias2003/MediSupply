...
/** Chat messages attached to a requisition or purchase order */
export const chatMessages = mysqlTable("chat_messages", {
  id: int("id").autoincrement().primaryKey(),
  /** The type of entity this message belongs to */
  entityType: mysqlEnum("entityType", ["requisition", "purchase_order"]).notNull(),
  /** ID of the requisition or purchase order */
  entityId: int("entityId").notNull(),
  userId: int("userId").notNull(),
  message: text("message").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => [index("idx_chat_entity").on(table.entityType, table.entityId)]);

export type ChatMessage = typeof chatMessages.$inferSelect;
export type InsertChatMessage = typeof chatMessages.$inferInsert;

/** Delivery receipt — pharmacist confirms physical receipt of delivered items */
export const deliveryReceipts = mysqlTable("delivery_receipts", {
  id: int("id").autoincrement().primaryKey(),
  poId: int("poId").notNull(),
  confirmedBy: int("confirmedBy").notNull(),   // pharmacist user id
  status: mysqlEnum("status", ["pending", "confirmed", "disputed"]).default("pending").notNull(),
  notes: text("notes"),
  confirmedAt: timestamp("confirmedAt"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => [index("idx_receipt_poId").on(table.poId)]);

export type DeliveryReceipt = typeof deliveryReceipts.$inferSelect;

/** Individual line items on a delivery receipt */
